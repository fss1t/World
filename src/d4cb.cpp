...
namespace
{
                             ...
                             const ForwardRealFFT *forward_real_fft, double *coarse_aperiodicity)
  {
    ...
    for (int i = 0; i < number_of_aperiodicities; ++i)
      coarse_aperiodicity[i] = MyMinDouble(1.0,
                                           coarse_aperiodicity[i] * pow(10.0, (current_f0 - 100) / 1000.0));
          ...
          int number_of_aperiodicities, const D4COption *option, double **aperiodicity)
{
  ...
  InitializeAperiodicity(f0_length, number_of_aperiodicities, aperiodicity);
